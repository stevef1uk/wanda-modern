...
@app.function(
    image=image,
    gpu=modal.gpu.A100(size="40GB"),  # Use A100 with 40GB (or size="80GB" for 80GB)
...
def test_gpu_pruning(vol: modal.Volume):
    ...
    # Check if this is the modified version by looking for --use_cpu flag
    with open("main.py", "r") as f:
        main_content = f.read()
        if "--use_cpu" in main_content:
